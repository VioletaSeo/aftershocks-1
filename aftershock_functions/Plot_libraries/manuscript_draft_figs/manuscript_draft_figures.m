...
completeness= 5; % Completeness of the catalog                (from KS test for Mid ocean rideges)
...
mscatTimeShifted = table(t,lat,lon,M,fms,res,prod,E,RT);
...
plot_ST_sensitiviety([inputCell,{'DepthRange',[0,maxDepth]}], CAT)
ftsz(gcf,8)
setsize(gcf,7,5)
...
%% 12) kn vs SVM
% I = ~isnan(FFIage);
plot_ML([(FFIage),(FFIdip), log10(areaNorm)],FFIres,c.Lat,c.Lon,c.Depth, ...
    lat,lon,depth,res)
setsize(gcf,5.2,2.4)
ftsz(gcf,8);
...
function plot_ST_sensitiviety(inputCell, CAT)
...
for n = 1:nT
    for m = 1:nDX
        ...
        subplot('Position',[(m-1)*0.9/nDX+0.07 (nT-n)*0.9/nT+0.07 1/nDX*0.85 1/nT*0.85])
        timeWin = {...
        ...
        [magArray,prodArray,kSFL,alphaSFL] = productivity_law(SENScat.M,ASinfoSENS.MSprod);
        scatter(magArray,prodArray,'sk','MarkerEdgeAlpha',0.5)
        p2 = plot(SENScat.M,kSFL*10.^(alphaSFL*SENScat.M),'k','LineWidth',2);
        
        % legends
        legend([p1,p2],{sprintf('N=10^{%0.1f M %0.1f}',alphaSENS, log10(kSENS)), ...
                sprintf('N_{SFL}=10^{%0.1f M %0.1f}',alphaSFL, log10(kSFL))}, ...
...
resPknn = knn_regress(loc,resAll,qloc,40);
hold on; 
...
switch N
    ...
    case 9 
        yyaxis right
        ylabel({'Fraction of mainshocks','with no aftershocks'})
end
